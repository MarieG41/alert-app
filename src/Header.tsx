...
export function Header() {
    ...
    return (
        <header className="text-center text-slate-50 bg-slate-900 h-40 p-5">
            <Form className="relativ text-right" action="/products">
                <input
                    type="search"
                    name="search"
                    placeholder="Search"
                    defaultValue={searchParams.get('search') ?? ''}
                    className="absolute right-10 top-10 rounded py-2 px-3 text-gray-700"
                />
            </Form>
            <Link to="">
                <img src={logo} alt="logo React" className="inline-block h-20" />
            </Link>
            <Link to="">
